    ...
    def sync_contraente(self, cr, uid, eib_db, move_id, eib_server, eib_ref):
        ...
        def _set_partner(partner_from_EIB):
            ...
            if partner_from_EIB['partitaIva']:
                ...
                if not contraente['datiAnagrafici']['nazioneCodiceIso']:
                    setup_vat = 'IT' + p_iva 
                else:
                    setup_vat = contraente['datiAnagrafici']['nazioneCodiceIso'] + p_iva 
            values = {
                    'is_company' : True,
                    'customer' : True,
                    #'ref' : element_to_import['ref'],
        ...
        # Contraente from EIB database (indirizzo principale x nazionalità e relativo controllo P.Iva)
        sql = "SELECT c.id, s.ragione_sociale, s.partita_iva, naz.iso_stato, s.codice_fiscale FROM CONTRAENTI C \
                LEFT JOIN ANAGRAFICHE_SOGGETTI S ON (s.id = c.sogg_id) \
                LEFT JOIN INDIRIZZI I ON (i.anso_id = c.sogg_id AND i.tpin_id = 1) \
                LEFT JOIN PAR_NAZIONI NAZ ON (naz.codice = i.pana_id ) \
        ...
        # Existing partner by vat
        if not rel_partner and eib_contraente[2]:
            domain = [('vat', 'like', '%' + eib_contraente[2])]
            partner_ids = partner_obj.search(cr, uid, domain, limit=1)
            if partner_ids:
                for partner in partner_obj.browse(cr, uid, partner_ids):
                    rel_partner_ids = rel_partner_obj.get_rel(cr, 
                                                              uid, 
                                                              partner.id, 
                                                              eib_partner_id = False)
                    # If relation doesn't exist, it will be created
                    if not rel_partner_ids:
                        rel_partner_id = rel_partner_obj.create_rel(cr, uid, partner.id, eib_contraente[0])  
                    else:
                        rel_partner_id = rel_partner_ids[0]
                    rel_partner = rel_partner_obj.browse(cr, uid, rel_partner_id)
                    log_desc += " - Esistente con P.I. %s " % (eib_contraente[2], )
        
        # Existing partner by fiscalcode
        if not rel_partner and eib_contraente[4]:
            domain = [('fiscalcode', 'like', '%' + eib_contraente[4])]
            partner_ids = partner_obj.search(cr, uid, domain, limit=1)
            if partner_ids:
                for partner in partner_obj.browse(cr, uid, partner_ids):
                    rel_partner_ids = rel_partner_obj.get_rel(cr, 
                    ...
                    if not rel_partner_ids:
                        ...
                        rel_partner_id = rel_partner_ids[0]
                    rel_partner = rel_partner_obj.browse(cr, uid, rel_partner_id)
                    log_desc += " - Esistente con C.F. %s " % (eib_contraente[4], )
        ...
        try:
            ...
            # Dati non passati da webservice
            contraente['datiAnagrafici']['nazioneCodiceIso'] = eib_contraente[3]
        except ValueError:
            error['state'] = True
        ...
        if not error['state'] and not rel_partner:
            rel_partner_id = rel_partner_obj.create_rel(cr, uid, partner_id, contraente['id'])  
...
class eib_conn_server(orm.Model):
    ...
    _columns = {
        ...
        #'partner_contraente_category_ids': fields.many2many(
        #                                            'res.partner.category', 
        #                                            'eib_conn_server_rel_contraente_category',
        #                                            'eib_conn_server_rel_contraente_category',
        #                                            'eib_conn_server_rel_contraente_category',
        #                                            string='Categories'),
...
class eib_conn_rel_partner(orm.Model):
    ...
    def create_rel(self, cr, uid, partner_id, eib_partner_id):
        
        if not partner_id or not eib_partner_id:
            return False
        # Test if already exists
        domain = [('partner_id', '=', partner_id), ('eib_partner_id', '=', eib_partner_id)]
    ...
    def get_rel(self, cr, uid, partner_id, eib_partner_id):
        
        domain = []
        if partner_id :
            domain.append(('partner_id', '=', partner_id))
        if eib_partner_id:
            domain.append(('eib_partner_id', '=', eib_partner_id))
        rel_ids = self.search(cr, uid, domain)
